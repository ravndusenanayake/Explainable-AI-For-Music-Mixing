      ...
      <div className="flex-1 overflow-y-auto p-2 space-y-2">
        ...
        <div className="bg-[#222] border border-[#333] rounded p-2.5">
          <div className="flex items-center justify-between mb-2">
            ...
            <Toggle checked={effects.eq.enabled} onChange={(val) => update('eq', { enabled: val })} />
          </div>
          {effects.eq.enabled && (
            <div className="space-y-1">
              <Slider label="Low-Cut" value={effects.eq.highPass} min={20} max={200} step={1} unit="Hz" onChange={(val) => update('eq', { highPass: val })} />
              <Slider label="Presence" value={effects.eq.presence} min={-5} max={10} step={0.5} unit="dB" onChange={(val) => update('eq', { presence: val })} />
            </div>
          )}
        </div>
        ...
        <div className="bg-[#222] border border-[#333] rounded p-2.5">
          <div className="flex items-center justify-between mb-2">
            ...
            <Toggle checked={effects.deEsser.enabled} onChange={(val) => update('deEsser', { enabled: val })} />
          </div>
          {effects.deEsser.enabled && (
            <Slider label="Amount" value={effects.deEsser.amount} min={0} max={100} step={1} unit="%" onChange={(val) => update('deEsser', { amount: val })} />
          )}
        </div>
        ...
        <div className="bg-[#222] border border-[#333] rounded p-2.5">
          <div className="flex items-center justify-between mb-2">
            ...
            <Toggle checked={effects.compressor.enabled} onChange={(val) => update('compressor', { enabled: val })} />
          </div>
          {effects.compressor.enabled && (
            <div className="space-y-1">
              <Slider label="Threshold" value={effects.compressor.threshold} min={-40} max={0} step={0.5} unit="dB" onChange={(val) => update('compressor', { threshold: val })} />
              <Slider label="Ratio" value={effects.compressor.ratio} min={1} max={10} step={0.1} unit=":1" onChange={(val) => update('compressor', { ratio: val })} />
            </div>
          )}
        </div>
        ...
        <div className="bg-[#222] border border-[#333] rounded p-2.5">
          <div className="flex items-center justify-between mb-2">
            ...
            <Toggle checked={effects.reverb.enabled} onChange={(val) => update('reverb', { enabled: val })} />
          </div>
          {effects.reverb.enabled && (
            <div className="space-y-2 mt-2">
              ...
              <Slider label="Mix" value={effects.reverb.mix} min={0} max={100} step={1} unit="%" onChange={(val) => update('reverb', { mix: val })} />
            </div>
          )}
        </div>
        ...
        <div className="bg-[#222] border border-[#333] rounded p-2.5">
          <div className="flex items-center justify-between mb-2">
            ...
            <Toggle checked={effects.delay.enabled} onChange={(val) => update('delay', { enabled: val })} />
          </div>
          {effects.delay.enabled && (
            <div className="space-y-2 mt-2">
              ...
              <Slider label="Mix" value={effects.delay.mix} min={0} max={100} step={1} unit="%" onChange={(val) => update('delay', { mix: val })} />
            </div>
          )}
        </div>
        ...
        <div className="bg-[#222] border border-[#333] rounded p-2.5">
          <div className="flex items-center justify-between mb-2">
            ...
            <Toggle checked={effects.saturation.enabled} onChange={(val) => update('saturation', { enabled: val })} />
          </div>
          {effects.saturation.enabled && (
            <Slider label="Drive" value={effects.saturation.drive} min={0} max={100} step={1} unit="%" onChange={(val) => update('saturation', { drive: val })} />
          )}
        </div>
